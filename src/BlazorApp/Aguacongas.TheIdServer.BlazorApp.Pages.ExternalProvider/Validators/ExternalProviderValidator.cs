...
namespace Aguacongas.TheIdServer.BlazorApp.Validators
{
    public class ExternalProviderValidator: AbstractValidator<ExternalProvider>
    {
        public ExternalProviderValidator(ExternalProvider externalProvider, IStringLocalizer localizer)
        {
            ...
            When(m => m.Options is RemoteAuthenticationOptions, () => 
            {
                RuleFor(m => m.Options as RemoteAuthenticationOptions).SetValidator(p => new RemoteAuthenticationOptionsValidator(externalProvider, localizer));
            });
